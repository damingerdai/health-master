...
export function HealthCharts({ summary }: HealthChartsProps) {
  const bloodPressureData = [...(summary.bloodPressureTrend ?? [])]
    .reverse()
    .map(item => ({
      date: new Date(item.logDatetime || '').toLocaleDateString('en-US', {
        month: 'short',
        day: 'numeric'
      }),
  ...
  const weightData = [...(summary.weightTrend ?? [])].reverse().map(item => ({
    date: new Date(item.recordDate).toLocaleDateString('en-US', {
      month: 'short',
      day: 'numeric'
    }),
    weight: item.weight
    ...
    <div className="grid gap-4 md:grid-cols-2">
      <Card>
        ...
        <CardContent>
          <ChartContainer
            config={{
              systolic: { label: 'Systolic', color: 'var(--color-chart-1)' },
              diastolic: { label: 'Diastolic', color: 'var(--color-chart-2)' }
            }}
      ...
      <Card>
        ...
        <CardContent>
          <ChartContainer
            config={{
              weight: { label: 'Weight (kg)', color: 'var(--color-chart-3)' }
            }}
            className="h-[300px] w-full"
          >
